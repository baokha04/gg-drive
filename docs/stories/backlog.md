...
| --- | --- | --- |
| E02-Auth | API key + per-IP rate limit on `POST /api/books/download`. | unsliced |
| E02-GDrive | Failure-retry, exponential backoff, dead-letter queue for the Google Drive upload integration. | unsliced |
