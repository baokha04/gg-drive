...
No book downloader module exists. The repository is a blank NestJS boilerplate template without database integration, file crawlers, or zipping utilities.
...
- Persists all operations in a local SQLite database (`database.db`) using three interconnected tables (`book`, `book_page`, `download_job`).
